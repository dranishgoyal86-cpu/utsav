...
const PHOTO_SHAPE_BY_LAYOUT = Object.freeze({
  ...
  // QA-pass fix: 'poster' (cultural-poster's only static layout) was
  // missing from this map entirely, so the hero image/artwork — the
  // brief's own first-listed poster requirement — silently never rendered
  // on the static card no matter what photo the host supplied. A taller
  // box than 'hero' (closer to square) so square exhibition artwork isn't
  // aggressively cropped away, while still reading fine for a landscape
  // concert/sports/festival photo.
  poster: 'poster',
  // Second missing-entry bug, same shape as the 'poster' one above:
  // 'minimal-spacious' (illustratedStory.js's only static layout — also
  // used by ivoryMandala.js/modernIndian.js/wellnessEarth.js) was never in
  // this map, so a host's photo silently never appeared on any of those
  // archetypes' cards, no matter what they uploaded. Found while
  // investigating why a kids-birthday child's photo wasn't showing on the
  // illustrated-story/storybook-party theme (Anish, Sept 17) — this map
  // is additive-only (a missing key just means "no photo slot", never a
  // rendering choice anyone relies on), so adding this key can only start
  // showing a photo where none rendered before; it changes nothing for
  // anyone not already trying to add a photo here. 'portrait' (a
  // pre-existing, already-styled shape — StaticInviteCard.js's
  // photoPortrait, 140x150) fits this layout's clean, compact aesthetic
  // better than the wider 'hero' or the ornate 'arch'.
  'minimal-spacious': 'portrait',
});
...
// decorationIcon: an optional single emoji/glyph rendered as a small
// corner badge (StaticInviteCard.js's existing s.iconBadge — this prop
// was already read there but nothing ever populated it before the Kids
// Birthday Theme-Aware Invite Designer wave; see
// lib/kidsBirthdayThemes.js). Kept generic here (not birthday-specific)
// so any future caller with a genuine per-event icon can reuse the same
// slot without a second badge mechanism.
export function buildStaticLayoutModel({ archetypeId, variantId, event, values, isNonFestive = false, qrTargetUrl = null, photoUrl = null, eventTypeSlug = null, decorationIcon = null } = {}) {
  ...
  const slots = {
    [STATIC_SLOT.DECORATION]: { motif: variant?.tokens?.motif || null, icon: decorationIcon || null },
    ...
    [STATIC_SLOT.KICKER]: values?.kickerText || values?.ceremonyName?.toUpperCase() || values?.ceremonyType?.toUpperCase() || values?.ceremonyCustomName?.toUpperCase() || values?.religiousEventType?.toUpperCase() || values?.sportName?.toUpperCase() || values?.genreNote?.toUpperCase() || null,
    [STATIC_SLOT.HEADLINE]: values?.headlineText || event?.name || null,
    [STATIC_SLOT.PRIMARY_NAMES]: buildPrimaryNames(values, eventTypeSlug),
    [STATIC_SLOT.HOST_LINE]: values?.hostedBy || values?.organiserName || null,
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .photo-poster { width: 100%; max-width: 460px; height: 340px; border-radius: 10px; }
