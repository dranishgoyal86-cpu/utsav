...
// step is deliberate: prasad-caterer is a distinct legacy_category_slug
// from catering, so scoping the drop to exactly legacy_category_slug ===
// 'catering' already leaves it untouched — a banquet kitchen bound to
// in-house catering cannot produce satvik prasad, so that requirement must
// always survive. legacy_category_slug (not category_slug) is used for
// both checks in this section deliberately — venues.provided_items still
// stores the old coarse vendor_categories-style slugs (unmigrated real
// data), while category_slug itself now holds real "Parent > Subcategory"
// taxonomy keys (see supabase/migrations/retire_vendor_categories_bridge.sql).
...
// 'inquiry' added alongside cancelled/payment_failed — a host who chose
// "Confirm with vendor first" hasn't actually committed to anything yet
// (no payment, no provider acceptance), so it must not count as "handled"
// for checklist-progress purposes any more than a cancelled booking would.
export const INACTIVE_BOOKING_STATUSES = new Set(['cancelled', 'payment_failed', 'inquiry']);
...
export function resolveRequirements(requirements, context = {}, venue = null) {
  const {
    ...
    isOutdoor = false, season = null, isHomeVenue = false,
  } = context;
  ...
  //
  // notHomeVenue — reported bug fix: lib/checklistContext.js's
  // buildChecklistContext() already computes isHomeVenue and
  // hooks/useEventPlan.js already spreads it into the context object this
  // function receives, but nothing here ever read it, so the seeded
  // "Venue" P1-essential row (every event type, event_requirements.sql,
  // condition_flag always null) showed as essential even when the host
  // picked "At home" and never booked/needed a venue. condition_flag only
  // supports "include when true," never "suppress when true" — so the
  // migration alongside this sets that row's condition_flag to
  // 'not_home_venue' (checked here as its negation) rather than inventing
  // a second, inverted condition mechanism.
  const conditionContext = {
    hasChildren, hasOutstationGuests, hasForeignGuests, hasPets, isOutdoor,
    isMonsoon: season === 'monsoon', isSummer: season === 'summer',
    notHomeVenue: !isHomeVenue,
  };
  ...
  const filtered = (requirements || []).filter(r => {
    ...
    if (venue) {
      if (providedItems.has(r.legacy_category_slug)) return false;
      if (inHouseCateringOnly && r.legacy_category_slug === 'catering') return false;
    }
...
// joins bookings -> services -> real match key before calling this — see
// vendorTaxonomy.js's resolveMatchKey() and hooks/useEventPlan.js — since
// this function stays Supabase-free and bookings has no category_slug
// column of its own). "Handled" uses the same broader not-cancelled/not-failed
