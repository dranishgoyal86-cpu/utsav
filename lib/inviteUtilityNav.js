...
// boostedSceneIds (Batch 4): an optional list of SCENE ids the CALLER
// considers this event's canonical primary action (e.g. 'tickets' for a
// concert, 'registration' for an exhibition) — ranked above every
// lifecyclePriority tier, never invented per-slug logic inside this
// resolver itself. Lets a genuinely different "what matters most" per
// event type shape the nav bar without a hardcoded per-slug array HERE —
// the branching (which id to boost for which event type) stays entirely
// in the content-mapping layer that calls this function, matching every
// other resolver in this registry ("stores none of the domain
// knowledge itself"). Ties within the boosted set, and within the
// unboosted remainder, still resolve by lifecyclePriority as before.
export function resolveUtilityNavFromScenes(scenes = [], { maxPrimary = 5, mode = LIFECYCLE_MODE.INVITATION, boostedSceneIds = [] } = {}) {
  ...
  const boosted = new Set(boostedSceneIds);
  candidates.sort((a, b) => {
    const aBoost = boosted.has(a.id) ? 1 : 0;
    const bBoost = boosted.has(b.id) ? 1 : 0;
    if (aBoost !== bBoost) return bBoost - aBoost;
    return PRIORITY_RANK.indexOf(b.lifecyclePriority[mode]) - PRIORITY_RANK.indexOf(a.lifecyclePriority[mode]);
  });
