...
// itself, so this list was built from the real distinct event_type_slug
// values in the live event_requirements table — it remains the source of
// truth for the app's actual event-creation taxonomy.
//
// The separate, pre-existing event_types table (built in an earlier pass of
// this app) was merged to match this list exactly as of 2026-09-01
// (supabase/migrations/event_types_merge.sql) — same 26 slugs, same order,
// with 4 then-stale slugs it used to carry (corporate-event, godh-bharai,
// griha-pravesh, satyanarayan-katha — zero matching rows anywhere) dropped.
// That table still exists only because GuestList.js does a real, live
// event_types.id → sub_events.event_type_id lookup to suggest function/
// sub-event names (Haldi/Sangeet/Reception-style); EVENT_TYPE_NAMES has no
// id column and can't serve that FK role. If you add a slug here, add the
// matching row to event_types too so that lookup stays in sync — see the
// migration file for the pattern.
