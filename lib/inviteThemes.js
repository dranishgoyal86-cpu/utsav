...
export const inviteThemes = {
  ...
  // Carries no tradition-specific symbols, same discipline as Kalamkari.
  // motif:'minimal' is its own render branch (two straight lines + huge
  // left-aligned type), not a reuse of arch/bloom.
  //
  // kicker here is the DEFAULT only — event_invite_content.kicker_text
  // (new, Wave 8) overrides it when a host sets one. The mockup's literal
  // "SAVE THE DATE" doesn't fit every wedding (same lesson as Toran's
  // "weds"), so "YOU'RE INVITED" ships as the real default instead of
  // silently keeping the mockup's string.
  ivory: {
    colors: {
      bg: '#FAF7F2',
      ink: '#1C1A17',
  ...
  // no gradient (flat cream, same "no glow" shape as Kalamkari) — accent
  // here is the rangoli/kicker red; flame amber and diya-bowl brown stay
  // literal in NightBloomCard-style fashion inside DiyaCard.js itself.
  diya: {
    colors: {
      bg: '#FDF4E3',
      ink: '#3A2416',
