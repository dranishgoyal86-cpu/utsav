...
// Plan-screen UX pass — "timings/duration of event should also be
// confirmed in the plan only, to be intimated to guests through invites".
// events.event_duration_hours (new, nullable) is the host's own optional
// "how long does it run" answer from EventTimeField — this is the one
// shared place that turns start time + duration into the range guests
// actually see, same reasoning as formatTimeLabel above (every consumer
// — invite designer, RSVPScreen.js, GuestList.js's invite payload —
// should read this instead of each re-deriving its own end time). Falls
// back to the plain start time when no duration was set, so nothing here
// forces the host to answer a question they haven't gotten to yet.
export function formatTimeRangeLabel(hhmm, durationHours) {
  const start = formatTimeLabel(hhmm);
  if (!start) return null;
  const hours = Number(durationHours);
  if (!hours || hours <= 0) return start;

  const [hStr, mStr] = hhmm.split(':');
  const startMinutes = parseInt(hStr, 10) * 60 + parseInt(mStr, 10);
  const endMinutes = Math.round(startMinutes + hours * 60) % (24 * 60);
  const endHH = String(Math.floor(endMinutes / 60)).padStart(2, '0');
  const endMM = String(endMinutes % 60).padStart(2, '0');
  const end = formatTimeLabel(`${endHH}:${endMM}`);
  return end ? `${start} – ${end}` : start;
}

// "Differentiation between kids birthday and normal birthday has to be
// placed in the event plan itself — whose birthday and birthdate, so we
// get to know the age... kids theme if below 15." Previously the only
// signal was matchEventTypeText() guessing from whatever the host typed
// when creating the event ("kids"/"daughter"/etc.) — no way to correct a
// wrong guess, and no signal at all if the text didn't happen to mention
// age. components/SlotField.js's new BirthdayPersonField collects a real
// birthdate on the Plan screen itself and calls these two pure helpers to
// turn it into an age, then an event type — shared here (not inlined in
// the field) so anything else that ever needs the same math agrees with it.
const KIDS_BIRTHDAY_AGE_CUTOFF = 15; // turning this age or older -> adult-birthday

export function computeAgeOn(dobStr, referenceDateStr) {
  ...
  const hadBirthdayByRef = ref.getMonth() > dob.getMonth() || (ref.getMonth() === dob.getMonth() && ref.getDate() >= dob.getDate());
  if (!hadBirthdayByRef) age -= 1;
  return age >= 0 ? age : null;
}

export function eventTypeForAge(age) {
  if (age == null) return null;
  return age < KIDS_BIRTHDAY_AGE_CUTOFF ? 'kids-birthday' : 'adult-birthday';
}
