...
//   - Kids Birthday Image Template pilot (Sept 17) — a third composite
//     string for a real illustrated image idea:
//     'image:<themeSlug>:<ideaId>', e.g. 'image:space-explorer:space-
//     explorer-1'. Same reasoning as the archetype prefix above: theme
//     slugs and idea ids are kebab-case, never contain ':'. themeSlug is
//     stored alongside ideaId (not just ideaId) so a saved selection can
//     be validated/looked up even if two themes ever reuse an idea id.
//     See lib/kidsBirthdayImageIdeas.js for the registry this resolves
//     against.
// No new column, no migration — a text column already accepts a longer
// string. parseProductionDesign()/buildArchetypeTemplateId()/
// buildImageTemplateId() below are the ONLY places that need to know this
// encoding; everything else (saveContent(), the `template_id` upsert, the
// initial load) treats `template_id` as an opaque string exactly as it
// always has.
...
import { getKidsBirthdayImageIdeaById } from './kidsBirthdayImageIdeas';
...
export function buildImageTemplateId(themeSlug, ideaId) {
  return `${IMAGE_PREFIX}${themeSlug}:${ideaId}`;
}

// Returns { kind: 'legacy', legacyDesignId } | { kind: 'archetype',
// archetypeId, variantId } | { kind: 'image', themeSlug, ideaId } |
// { kind: 'none' } — never throws, never returns a partially-parsed shape
...
export function parseProductionDesign(templateId) {
  ...
  if (templateId.startsWith(IMAGE_PREFIX)) {
    const rest = templateId.slice(IMAGE_PREFIX.length);
    const sep = rest.indexOf(':');
    if (sep === -1) return { kind: 'none' };
    const themeSlug = rest.slice(0, sep);
    const ideaId = rest.slice(sep + 1);
    if (!themeSlug || !ideaId) return { kind: 'none' };
    return { kind: 'image', themeSlug, ideaId };
  }
  return { kind: 'legacy', legacyDesignId: templateId };
}

// Same defensive contract as isValidArchetypeSelection below — a saved
// image selection is only "valid" while its exact (themeSlug, ideaId)
// pair still exists in the registry (e.g. never crashes if an idea were
// ever renamed/removed later).
export function isValidImageSelection(themeSlug, ideaId) {
  return !!getKidsBirthdayImageIdeaById(themeSlug, ideaId);
}
