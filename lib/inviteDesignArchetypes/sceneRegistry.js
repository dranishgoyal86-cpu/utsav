...
export const SCENE_REGISTRY = Object.freeze({
  ...
  [SCENE_ROLE.HONOUREE]: scene({
    ...
    lifecyclePriority: { [M.INVITATION]: P.HIGH, [M.PRE_EVENT]: P.LOW, [M.EVENT_DAY]: P.LOW, [M.POST_EVENT]: P.LOW },
    densityContribution: 'low', implementedAs: 'honouree', // Production Batch 1 — Kids Birthday
  }),
  ...
  [SCENE_ROLE.TRANSPORT]: scene({
    ...
    densityContribution: 'medium', implementedAs: 'transport', // Production Batch 3
  }),
  [SCENE_ROLE.GUEST_ACCESS]: scene({
    ...
    // Batch 2 carry-forward fix — this role had NO navigationLabel, so it
    // could never appear in the persistent nav bar at all regardless of
    // priority (resolveUtilityNavFromScenes() skips any candidate with no
    // navigationLabel). Housewarming in particular needs Gate Pass to be
    // reachable one tap away on event day, not buried — see this role's
    // own VERY_HIGH event-day priority just below, which was already
    // correctly declared but had no way to ever surface.
    navigationLabel: 'Gate',
  ...
  [SCENE_ROLE.DRESS_CODE]: scene({
    ...
    lifecyclePriority: { [M.INVITATION]: P.LOW, [M.PRE_EVENT]: P.HIGH, [M.EVENT_DAY]: P.MEDIUM, [M.POST_EVENT]: P.NONE },
    densityContribution: 'low', implementedAs: 'dress-code', // Production Batch 1 — Engagement/Nikah/weddings
  }),
  ...
  [SCENE_ROLE.REGISTRATION]: scene({
    ...
    lifecyclePriority: { [M.INVITATION]: P.HIGH, [M.PRE_EVENT]: P.MEDIUM, [M.EVENT_DAY]: P.HIGH, [M.POST_EVENT]: P.NONE },
    densityContribution: 'medium', implementedAs: 'registration', // Production Batch 2
  }),
  [SCENE_ROLE.SPEAKERS]: scene({
    id: SCENE_ROLE.SPEAKERS, role: 'Speaker/panelist presentation (corporate-conference; Exhibition/Product Launch may reuse later)',
    navigationLabel: 'Speakers',
    lifecyclePriority: { [M.INVITATION]: P.MEDIUM, [M.PRE_EVENT]: P.MEDIUM, [M.EVENT_DAY]: P.HIGH, [M.POST_EVENT]: P.NONE },
    densityContribution: 'medium', implementedAs: 'speakers', // Production Batch 2
  }),
  ...
  [SCENE_ROLE.TICKETS]: scene({
    ...
    lifecyclePriority: { [M.INVITATION]: P.HIGH, [M.PRE_EVENT]: P.MEDIUM, [M.EVENT_DAY]: P.HIGH, [M.POST_EVENT]: P.NONE },
    densityContribution: 'low', implementedAs: 'tickets', // Production Batch 4
  }),
  ...
  [SCENE_ROLE.CONTACT]: scene({
    ...
    densityContribution: 'none', implementedAs: 'contact', // Production Batch 3
  }),
