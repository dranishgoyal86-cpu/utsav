...
// Foundation wave. Distinct from (but cross-referencing) the full
// archetype objects in archetypes/*.js: this file exists so Utsav can
// answer compatibility questions for all 18 target directions named in
// the brief. As of Production Batch 4 (Exhibition, Concert, Festival/
// Fair, Sports Event), 17 entries are IMPLEMENTED (Batches 1-3's 16 +
// cultural-poster, Utsav's primary public-event direction) and only
// 'stillness' stays 'planned' — pure metadata, never selectable in
// production, see lib/inviteDesignCompatibilityMatrix.js's
// getSelectableArchetypes()).
//
...
export const EVENT_STRONG_ARCHETYPES = Object.freeze({
  ...
  // Production Batch 1 — 'ivory-mandala' deliberately removed from this
  // list (was present in the Design System Scaling Foundation wave's
  // original brief transcription). "Mandala" carries a Hindu/Dharmic
  // religious connotation unsuited to a Nikah context even though the
  // underlying geometry is visually neutral — see
  // archetypes/ivoryMandala.js's own header comment. Nikah's real,
  // religiously-safe options are mughal-garden (primary), night-bloom and
  // botanical-romance (contemporary), all with no Hindu/Islamic
  // iconography baked into their motifs.
  'nikah': ['mughal-garden', 'botanical-romance', 'photo-editorial', 'night-bloom', 'modern-indian'],
  ...
  // Batch 2 — folk-celebration added so Godh Bharai/Seemantham/Valaikappu
  // hosts have a respectful traditional direction alongside the 4
  // contemporary ones (was missing from this wave's own earlier
  // transcription; the ceremony-terminology fields already existed on
  // the baby-shower schema, this catalogue entry just hadn't caught up).
  'baby-shower': ['botanical-romance', 'celestial', 'illustrated-story', 'ivory-mandala', 'photo-editorial', 'folk-celebration'],
...
export const CATALOGUE = Object.freeze({
  ...
  'mughal-garden': entry({
    id: 'mughal-garden', name: 'Mughal Garden', status: ARCHETYPE_STATUS.IMPLEMENTED,
    density: { min: CONTENT_DENSITY.MEDIUM, max: CONTENT_DENSITY.RICH },
    tones: ['traditional', 'formal', 'romantic'],
    variantIds: ['emerald-mehfil'], staticLayoutFamilies: [STATIC_LAYOUT_FAMILY.FRAMED_PORTRAIT],
    defaultMotionPreset: MOTION_PRESET.UNVEIL, supportedMotionPresets: [MOTION_PRESET.UNVEIL, MOTION_PRESET.DRIFT],
  }),
  'temple-heritage': entry({
    id: 'temple-heritage', name: 'Temple Heritage', status: ARCHETYPE_STATUS.IMPLEMENTED,
    ...
    variantIds: ['sacred-threshold'], staticLayoutFamilies: [STATIC_LAYOUT_FAMILY.CENTERED_CEREMONIAL],
    defaultMotionPreset: MOTION_PRESET.UNVEIL, supportedMotionPresets: [MOTION_PRESET.UNVEIL, MOTION_PRESET.DRIFT],
  }),
  'folk-celebration': entry({
    id: 'folk-celebration', name: 'Folk Celebration', status: ARCHETYPE_STATUS.IMPLEMENTED,
    density: { min: CONTENT_DENSITY.MEDIUM, max: CONTENT_DENSITY.VERY_RICH },
    tones: ['warm', 'energetic', 'traditional'],
    variantIds: ['festive-blessing'], staticLayoutFamilies: [STATIC_LAYOUT_FAMILY.CENTERED_CEREMONIAL],
    defaultMotionPreset: MOTION_PRESET.SPARK, supportedMotionPresets: [MOTION_PRESET.SPARK, MOTION_PRESET.DRIFT],
  }),
  'botanical-romance': entry({
    id: 'botanical-romance', name: 'Botanical Romance', status: ARCHETYPE_STATUS.IMPLEMENTED,
    density: { min: CONTENT_DENSITY.LIGHT, max: CONTENT_DENSITY.RICH },
    tones: ['romantic', 'warm', 'modern'], requiresPhotography: true,
    variantIds: ['rose-garden'], staticLayoutFamilies: [STATIC_LAYOUT_FAMILY.SPLIT_PHOTO],
    defaultMotionPreset: MOTION_PRESET.DRIFT, supportedMotionPresets: [MOTION_PRESET.DRIFT, MOTION_PRESET.BLOOM],
  }),
  'illustrated-story': entry({
    id: 'illustrated-story', name: 'Illustrated Story', status: ARCHETYPE_STATUS.IMPLEMENTED,
    density: { min: CONTENT_DENSITY.LIGHT, max: CONTENT_DENSITY.MEDIUM },
    tones: ['playful', 'warm', 'whimsical'], supportsIllustration: true,
    variantIds: ['storybook-party'], staticLayoutFamilies: [STATIC_LAYOUT_FAMILY.MINIMAL_SPACIOUS],
    defaultMotionPreset: MOTION_PRESET.DRIFT, supportedMotionPresets: [MOTION_PRESET.DRIFT],
  }),
  'photo-editorial': entry({
    id: 'photo-editorial', name: 'Photo Editorial', status: ARCHETYPE_STATUS.IMPLEMENTED,
    density: { min: CONTENT_DENSITY.LIGHT, max: CONTENT_DENSITY.RICH },
    tones: ['modern', 'minimal', 'formal'], requiresPhotography: true,
    capabilitiesPresented: capabilities({ invocation: false }),
    variantIds: ['editorial-ivory'], staticLayoutFamilies: [STATIC_LAYOUT_FAMILY.PHOTO_EDITORIAL],
    defaultMotionPreset: MOTION_PRESET.DRIFT, supportedMotionPresets: [MOTION_PRESET.DRIFT],
  }),
  'modern-indian': entry({
    id: 'modern-indian', name: 'Modern Indian', status: ARCHETYPE_STATUS.IMPLEMENTED,
    density: { min: CONTENT_DENSITY.LIGHT, max: CONTENT_DENSITY.RICH },
    tones: ['modern', 'minimal', 'formal'],
    variantIds: ['modern-home'], staticLayoutFamilies: [STATIC_LAYOUT_FAMILY.MINIMAL_SPACIOUS],
    defaultMotionPreset: MOTION_PRESET.DRIFT, supportedMotionPresets: [MOTION_PRESET.DRIFT],
  }),
  'night-bloom': entry({
    id: 'night-bloom', name: 'Night Bloom', status: ARCHETYPE_STATUS.IMPLEMENTED,
    density: { min: CONTENT_DENSITY.MEDIUM, max: CONTENT_DENSITY.RICH },
    tones: ['romantic', 'modern', 'energetic'],
    variantIds: ['midnight-jasmine'], staticLayoutFamilies: [STATIC_LAYOUT_FAMILY.CENTERED_CEREMONIAL],
    defaultMotionPreset: MOTION_PRESET.DRIFT, supportedMotionPresets: [MOTION_PRESET.DRIFT, MOTION_PRESET.BLOOM],
  }),
  'celestial': entry({
    id: 'celestial', name: 'Celestial', status: ARCHETYPE_STATUS.IMPLEMENTED,
    density: { min: CONTENT_DENSITY.LIGHT, max: CONTENT_DENSITY.MEDIUM },
    tones: ['calm', 'playful', 'whimsical'],
    capabilitiesPresented: capabilities({ travel: false, accommodation: false, gatePass: false }),
    variantIds: ['space-adventure'], staticLayoutFamilies: [STATIC_LAYOUT_FAMILY.POSTER],
    defaultMotionPreset: MOTION_PRESET.DRIFT, supportedMotionPresets: [MOTION_PRESET.DRIFT, MOTION_PRESET.SPARK],
  }),
  'playful-pop': entry({
    id: 'playful-pop', name: 'Playful Pop', status: ARCHETYPE_STATUS.IMPLEMENTED,
    density: { min: CONTENT_DENSITY.LIGHT, max: CONTENT_DENSITY.MEDIUM },
    tones: ['playful', 'energetic', 'warm'],
    capabilitiesPresented: capabilities({ invocation: false, travel: false, accommodation: false }),
    variantIds: ['confetti-pop', 'comic-burst'], staticLayoutFamilies: [STATIC_LAYOUT_FAMILY.POSTER],
    defaultMotionPreset: MOTION_PRESET.SPARK, supportedMotionPresets: [MOTION_PRESET.SPARK, MOTION_PRESET.DRIFT],
  }),
  'luxury-black': entry({
    id: 'luxury-black', name: 'Luxury Black', status: ARCHETYPE_STATUS.IMPLEMENTED,
    density: { min: CONTENT_DENSITY.MEDIUM, max: CONTENT_DENSITY.RICH },
    tones: ['formal', 'modern', 'minimal'],
    capabilitiesPresented: capabilities({ invocation: false, gifts: false, wishingWall: false }),
    variantIds: ['spotlight-black'], staticLayoutFamilies: [STATIC_LAYOUT_FAMILY.PHOTO_EDITORIAL],
    defaultMotionPreset: MOTION_PRESET.UNVEIL, supportedMotionPresets: [MOTION_PRESET.UNVEIL, MOTION_PRESET.DRIFT],
  }),
  'corporate-grid': entry({
    id: 'corporate-grid', name: 'Corporate Grid', status: ARCHETYPE_STATUS.IMPLEMENTED,
    density: { min: CONTENT_DENSITY.LIGHT, max: CONTENT_DENSITY.RICH },
    ...
    variantIds: ['executive-light', 'summit-dark'], staticLayoutFamilies: [STATIC_LAYOUT_FAMILY.PROGRAMME_POSTER],
    defaultMotionPreset: MOTION_PRESET.DRIFT, supportedMotionPresets: [MOTION_PRESET.DRIFT],
  }),
  'cultural-poster': entry({
    id: 'cultural-poster', name: 'Cultural Poster', status: ARCHETYPE_STATUS.IMPLEMENTED,
    // QA-pass fix: a real exhibition/concert/festival/sports fixture with a
    // schedule + a few notes computes as RICH, which pushed this out of its
    // own "primary/strong" slot for exactly the events it's meant to lead
    // (see EVENT_STRONG_ARCHETYPES above) — the poster static layout stays
    // sparse regardless of how much data backs it, so it can absorb richer
    // content the same way photo-editorial/modern-indian/luxury-black do.
    density: { min: CONTENT_DENSITY.LIGHT, max: CONTENT_DENSITY.RICH },
    ...
    variantIds: ['gallery-poster', 'live-poster'], staticLayoutFamilies: [STATIC_LAYOUT_FAMILY.POSTER],
    defaultMotionPreset: MOTION_PRESET.SPARK, supportedMotionPresets: [MOTION_PRESET.SPARK, MOTION_PRESET.DRIFT],
  }),
  'stillness': entry({
  ...
  'wellness-earth': entry({
    id: 'wellness-earth', name: 'Wellness Earth', status: ARCHETYPE_STATUS.IMPLEMENTED,
    density: { min: CONTENT_DENSITY.LIGHT, max: CONTENT_DENSITY.MEDIUM },
    tones: ['calm', 'warm', 'minimal'],
    capabilitiesPresented: capabilities({ invocation: false, gifts: false, gatePass: false }),
    variantIds: ['forest-retreat'], staticLayoutFamilies: [STATIC_LAYOUT_FAMILY.MINIMAL_SPACIOUS],
    defaultMotionPreset: MOTION_PRESET.DRIFT, supportedMotionPresets: [MOTION_PRESET.DRIFT, MOTION_PRESET.BLOOM],
  }),
});
