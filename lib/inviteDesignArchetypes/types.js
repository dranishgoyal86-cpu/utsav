...
export const MOTION_PRESET = Object.freeze({
  ...
  STILLNESS: 'stillness',
  // Production Batch 1 — the one new preset this wave adds, for Playful
  // Pop/Celestial (kids-birthday). Added only because it's genuinely
  // reusable across any future playful/celebratory archetype (per the
  // brief's own "add only if truly reusable" rule), not scoped to one
  // component's local animation logic.
  SPARK: 'spark',
});
...
export const SCENE = Object.freeze({
  ...
  CLOSING: 'closing',
  // Production Batch 1 — 2 real additions to this working, wired-up enum
  // (not a new registry — same file, same object, same pattern every
  // resolver here already follows). Both already existed as contract-only
  // SCENE_ROLE entries in sceneRegistry.js (DRESS_CODE, HONOUREE); this is
  // their first real implementation, needed for genuine Engagement/Nikah
  // (dress code) and Kids Birthday (single-honouree identity) production
  // composition — not invented fresh.
  DRESS_CODE: 'dress-code',
  HONOUREE: 'honouree',
  // Production Batch 2 — 2 more real additions, same pattern as
  // DRESS_CODE/HONOUREE above: both already existed as contract-only
  // SCENE_ROLE entries (REGISTRATION, SPEAKERS) since the Design System
  // Scaling Foundation wave; this is their first real implementation,
  // needed for genuine Corporate Conference/Product Launch composition.
  REGISTRATION: 'registration',
  SPEAKERS: 'speakers',
  // Production Batch 3 — same pattern again: both already existed as
  // contract-only SCENE_ROLE entries; first real implementation, needed
  // for Team Offsite/Wellness Retreat.
  TRANSPORT: 'transport',
  CONTACT: 'contact',
  // Production Batch 4 — same pattern, needed for Exhibition/Concert/
  // Festival/Sports ticket-or-entry presentation.
  TICKETS: 'tickets',
});
...
export const STATIC_SLOT = Object.freeze({
  ...
  // Production Batch 1 — a host-supplied photo (hero/heroPhoto field on
  // the event schema), only populated for archetypes whose static.layouts
  // actually use a photo (split-photo, photo-editorial). Carries a crop
  // `shape` hint (hero/portrait/circle/arch) resolved from the layout
  // family, not stored per-event — the same photo works across any
  // archetype the host later switches to.
  PHOTO: 'photo',
});
