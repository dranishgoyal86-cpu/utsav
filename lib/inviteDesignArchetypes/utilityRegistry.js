...
export const UTILITY_REGISTRY = Object.freeze({
  ...
  'gate-pass-card': utility({
    id: 'gate-pass-card', name: 'GatePassCard', status: ARCHETYPE_STATUS.IMPLEMENTED,
    sceneRoleId: SCENE_ROLE.GUEST_ACCESS, themeTokenKeys: STANDARD_UTILITY_TOKENS,
    componentPath: 'components/inviteArchetypes/utility/GatePassCard.js',
  }),
  'dress-code-card': utility({
    id: 'dress-code-card', name: 'DressCodeCard', status: ARCHETYPE_STATUS.IMPLEMENTED,
    sceneRoleId: SCENE_ROLE.DRESS_CODE, themeTokenKeys: STANDARD_UTILITY_TOKENS,
    componentPath: 'components/inviteArchetypes/utility/DressCodeCard.js',
  }),
  'wishing-wall-card': utility({
    id: 'wishing-wall-card', name: 'WishingWallCard', status: ARCHETYPE_STATUS.IMPLEMENTED,
    sceneRoleId: SCENE_ROLE.WISHING_WALL, themeTokenKeys: STANDARD_UTILITY_TOKENS,
    componentPath: 'components/inviteArchetypes/utility/WishingWallCard.js',
  }),
  // Production Batch 2 — corporate-conference/product-launch.
  'registration-card': utility({
    id: 'registration-card', name: 'RegistrationCard', status: ARCHETYPE_STATUS.IMPLEMENTED,
    sceneRoleId: SCENE_ROLE.REGISTRATION, themeTokenKeys: STANDARD_UTILITY_TOKENS,
    componentPath: 'components/inviteArchetypes/utility/RegistrationCard.js',
  }),
  'speaker-card': utility({
    id: 'speaker-card', name: 'SpeakersScene', status: ARCHETYPE_STATUS.IMPLEMENTED,
    sceneRoleId: SCENE_ROLE.SPEAKERS, themeTokenKeys: STANDARD_UTILITY_TOKENS,
    componentPath: 'components/inviteArchetypes/scenes/SpeakersScene.js',
  }),

  // Production Batch 3 — team-offsite/wellness-retreat.
  'transport-card': utility({
    id: 'transport-card', name: 'TransportCard', status: ARCHETYPE_STATUS.IMPLEMENTED,
    sceneRoleId: SCENE_ROLE.TRANSPORT, themeTokenKeys: STANDARD_UTILITY_TOKENS,
    componentPath: 'components/inviteArchetypes/utility/TransportCard.js',
  }),
  'contact-card': utility({
    id: 'contact-card', name: 'ContactCard', status: ARCHETYPE_STATUS.IMPLEMENTED,
    sceneRoleId: SCENE_ROLE.CONTACT, themeTokenKeys: STANDARD_UTILITY_TOKENS,
    componentPath: 'components/inviteArchetypes/utility/ContactCard.js',
  }),

  // Production Batch 4 — exhibition/concert/festival-fair/sports-event.
  'ticket-card': utility({
    id: 'ticket-card', name: 'TicketCard', status: ARCHETYPE_STATUS.IMPLEMENTED,
    sceneRoleId: SCENE_ROLE.TICKETS, themeTokenKeys: STANDARD_UTILITY_TOKENS,
    componentPath: 'components/inviteArchetypes/utility/TicketCard.js',
  }),
  // ScheduleCard's role (performances/fixtures/festival sessions/talks) is
  // already fully served by FunctionCard's own event_functions-backed
  // list + its Batch 2 `title` override ("Agenda"/"Programme"/"Fixtures")
  // — no second list-rendering component ("do not build four
  // event-specific schedule components").
  'schedule-card': utility({
    id: 'schedule-card', name: 'ScheduleCard (served by FunctionCard)', status: ARCHETYPE_STATUS.IMPLEMENTED,
    sceneRoleId: SCENE_ROLE.PROGRAMME, themeTokenKeys: STANDARD_UTILITY_TOKENS,
    componentPath: 'components/inviteArchetypes/utility/FunctionCard.js',
  }),
