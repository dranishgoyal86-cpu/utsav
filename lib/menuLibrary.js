...
// icon — a plain emoji, not an icon-font glyph (see
// claude/menu-planning-vs-execution-split.md for why: no package/import to
// get wrong, renders identically on iOS/Android/web). Menu Stage split
// (menu_stage on events, 20260917000000_menu_planning_execution.sql) is a
// separate, purely display-layer change — doesn't touch this catalog data
// at all.
export const MENU_CATEGORIES = [
  { slug: 'welcome-drinks', label: 'Welcome Drinks', icon: '🥤' },
  { slug: 'starters', label: 'Starters', icon: '🍢' },
  { slug: 'chaat-street-food', label: 'Chaat / Street Food', icon: '🌮' },
  { slug: 'main-course', label: 'Main Course', icon: '🍛' },
  { slug: 'breads', label: 'Breads', icon: '🫓' },
  { slug: 'rice-biryani', label: 'Rice / Biryani', icon: '🍚' },
  { slug: 'dal-curry', label: 'Dal / Curry', icon: '🍲' },
  { slug: 'accompaniments', label: 'Accompaniments', icon: '🥗' },
  { slug: 'kids-menu', label: 'Kids Menu', icon: '🧒' },
  { slug: 'live-counters', label: 'Live Counters', icon: '🔥' },
  { slug: 'desserts', label: 'Desserts', icon: '🍰' },
  { slug: 'tea-coffee', label: 'Tea / Coffee', icon: '☕' },
  { slug: 'packaged-items', label: 'Packaged Items', icon: '📦' },
];
...
// Small veg/egg/non-veg classifier behind the standard Indian
// green-square/red-square dot marker (MenuLibrary.js's VegDot). Takes a
// catalog dish's foodTypes array directly, or a saved event_menu_selections
// row's own is_veg/food_type columns via dishVegStatusFromSelection below —
// two entry points because the two call sites have different shapes, same
// classification logic either way.
export function dishVegStatus(foodTypes = []) {
  if (foodTypes.includes('non-veg')) return 'non-veg';
  if (foodTypes.includes('veg-egg')) return 'egg';
  return 'veg';
}

export function dishVegStatusFromSelection(sel) {
  if (sel.food_type === 'veg-egg') return 'egg';
  return sel.is_veg === false ? 'non-veg' : 'veg';
}
