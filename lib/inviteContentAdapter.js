...
import { FIELD_STATUS } from './inviteSchemas/types';
...
// Visual Production Batch 2 — a CONDITIONAL field (naming-ceremony's
// babyName/nameIsSecret, product-launch's productName/productNameHidden,
// nikah's bismillahText/quranicVerseText's own conditionOn pattern) has
// always had its condition() predicate available (lib/inviteSchemas/
// types.js's field()), but nothing actually APPLIED it at render time
// before this — a CONDITIONAL field's value simply passed straight
// through normalizeInviteContent() like any other field, which is a real
// leak risk: `nameIsSecret: true` alone never stopped `values.babyName`
// from reaching a renderer if that value happened to still be present in
// schema_content (e.g. typed in before the host later marked the name
// secret). This is the resolver-level safeguard the brief requires —
// suppression happens ONCE, here, for every schema/every caller, not
// re-implemented per event type or left to each renderer to remember.
function applyConditionalSuppression(schema, values) {
  const fields = listSchemaFields(schema);
  const result = { ...values };
  for (const f of fields) {
    if (f.status === FIELD_STATUS.CONDITIONAL && typeof f.condition === 'function' && !f.condition(result)) {
      result[f.key] = '';
    }
  }
  return result;
...
export function normalizeInviteContent(schema, contentRow) {
  ...
  return applyConditionalSuppression(schema, values);
}
...
// Launch-readiness audit fix — riteType (the field whose own placeholder
// names Chautha/Uthala/Tehravi/Bhog/Last Rites/Cremation/Burial/Prayer
// Meeting) and familyContactInfo (a REQUIRED field on this schema) had
// been collected by the builder form since this schema's original
// authoring but never appeared on the rendered card at all — StillnessCard
// had an empty slot already reserved for exactly this ("functionName",
// previously only used for a per-function card, blank on the main event
// card) plus room for one more short contact line. Fixing the mapping
// here is enough; ToranInvites.js/GuestList.js already spread this
// function's return value straight into <StillnessCard />, so no call
...
export function mapToStillnessCardProps(values, functionOverride) {
  ...
  return {
    ...
    functionDate: functionOverride?.date,
    functionTime: functionOverride?.time,
    riteType: functionOverride ? undefined : (v.riteType || ''),
    contactInfo: functionOverride ? undefined : (v.familyContactInfo || ''),
  };
}
