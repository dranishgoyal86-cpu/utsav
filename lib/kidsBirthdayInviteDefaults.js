...
import { getKidsBirthdayInviteCopy } from './kidsBirthdayInviteCopy';
import { KIDS_BIRTHDAY_FAMILY_MESSAGES } from './kidsBirthdayFamilyMessages';
...
// reused here, not refetched or reimplemented). opts.hostNamePref: the
// host's own saved name from users.invite_preferences.hostName — the
// SAME source ToranInvites.js's own pre-fill (for partner_1_name on
// couple-shaped invites) already reads; passed in here rather than
// refetched, so kids-birthday's "hosted by" line reuses one real "if
// shared before" source rather than inventing a second one (Anish, Sept
// 17: "hosted by - if shared before").
export function buildKidsBirthdayInviteDefaults(event, { featuredActivities = [], hostNamePref = null } = {}) {
  if (!event) return {};
  const defaults = {};
  ...
  if (hostNamePref) {
    defaults.hostedBy = hostNamePref;
  }
  ...
  if (event.theme_slug) {
    ...
    // Anish, Sept 17: "tagline- auto fill according to theme, kicker
    // text- auto generate." Both are silent defaults, same as every
    // other field here — only fills an empty field, a host's own edit
    // always wins on reopen (see this file's own precedence note above).
    const copy = getKidsBirthdayInviteCopy(event.theme_slug);
    defaults.tagline = copy.tagline;
    defaults.kickerText = copy.kickerText;
  }

  // "A message from the family" — Anish, Sept 17: "keep 10 messages in
  // the library- host can change them." The library itself is offered as
  // tappable suggestions in the designer (see InviteFieldRenderer.js's
  // `suggestions` prop, wired in ToranInvites.js) so the host actively
  // picks one; this default only seeds the FIRST message so the field is
  // never blank for a brand-new invite, exactly like every other default
  // here — a host who never touches this field still sends something
  // warm, and one that picks a different message or edits it keeps that
  // choice on reopen (never overwritten).
  defaults.customMessage = KIDS_BIRTHDAY_FAMILY_MESSAGES[0];
