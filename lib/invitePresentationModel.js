...
export function buildStoryText(eventTypeSlug, values, featuredActivities = []) {
  const lines = [];
  ...
  if (eventTypeSlug === 'nikah') {
    ...
    if (values.activitiesNote) lines.push(values.activitiesNote);
    // Birthday Event Improvement plan, Piece 4 — real, host-curated data
    // (event_extra_activities rows the host starred "feature on invite"),
    // not a duplicate of the free-text activitiesNote above — a host can
    // have either, both, or neither. See lib/activityIdeas.js and
    // components/ActivityIdeasLibrary.js for where these come from.
    if (featuredActivities.length > 0) lines.push(`What to expect: ${featuredActivities.join(', ')}`);
...
// builders.
export function buildPresentationContent({ eventTypeSlug, values, event, functions = [], featuredActivities = [] } = {}) {
  ...
  const storyText = buildStoryText(eventTypeSlug, values, featuredActivities);
