...
- Razorpay credentials: `create-razorpay-order` (deployed edge function) calls Razorpay's Orders API with the `RAZORPAY_KEY_ID`/`RAZORPAY_KEY_SECRET` Supabase secrets and gets "Authentication failed" back from Razorpay itself — pre-existing (two failed test bookings from 2026-06-24 show the same failure, before any of this session's changes). Not an app-code issue; check/regenerate the key pair on the Razorpay dashboard and re-set via `supabase secrets set`.
